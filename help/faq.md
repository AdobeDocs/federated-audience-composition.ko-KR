...
다음은 Adobe Experience Platform 페더레이션된 대상자 컴포지션과 관련하여 자주 묻는 질문 목록입니다. [이 페이지](https://experienceleague.adobe.com/ko/docs/experience-platform/segmentation/faq){target="_blank"}에서는 Adobe Experience Platform Segmentation Service에 대한 글로벌 FAQ도 제공됩니다.

## Federated Audience Composition에 액세스하려면 어떤 권한이 필요합니까?

+++ 답변

페더레이션된 대상자 컴포지션을 위해서는 Adobe Real-Time Customer Data Platform 및 Adobe Journey Optimizer Prime 또는 Ultimate 패키지가 필요합니다. 또한 페더레이션된 대상자 컴포지션을 구매해야 합니다.

페더레이션된 대상자 컴포지션을 사용하려면 각 사용자를 각 샌드박스에 대해 생성된 특정 프로필에 추가해야 합니다. 자세한 내용은 [페더레이션된 대상자 컴포지션 액세스](./start/access-prerequisites.md) 페이지를 참조하십시오.

+++

## 지원되는 클라우드 웨어하우스는 무엇입니까?

+++ 답변

현재 지원되는 클라우드 웨어하우스는 다음과 같습니다.
...
Data Warehouse 연결에 대한 자세한 내용은 [연결 개요](./connections/home.md)를 참조하십시오.
...
예, 여러 개의 데이터 웨어하우스를 동일한 컴포지션으로 쿼리할 수 있으며 여러 소스의 데이터를 결합할 수 있습니다.  일반적으로 각 [컴포지션 활동](./compositions/activities.md)(쿼리, 보강, 분할 등)은 활동 구성, 대상 데이터베이스(여러 개의 페더레이션된 데이터 액세스 사례가 있을 수 있음) 및 실행 결과가 포함된 하나 이상의 작업 테이블의 출력에 따라 하나 또는 여러 개의 SQL 문을 실행합니다. 해당 작업 테이블은 연속적인 활동을 위한 입력으로 사용됩니다.

+++

### 페더레이션된 대상자 컴포지션을 사용하여 전체 데이터베이스에 액세스할 수 있습니까?

+++ 답변

아니요, 전용 또는 공유 데이터베이스/스키마에 대한 액세스를 구성하는 것은 사용자에게 달려 있습니다. 페더레이션된 대상자 컴포지션을 위한 전용 스키마를 만들고 비즈니스 사례 데이터 세트만 복사/공유하는 것이 좋습니다.
+++
...
예, 일단 연결되면 페더레이션된 대상자 컴포지션을 사용하여 정의된 초기 권한을 기반으로 모든 테이블을 검색할 수 있으며, 시각적 스키마 편집기를 사용하여 다음 작업을 수행할 수 있습니다.
...
아니요. 페더레이션된 대상자 컴포지션은 메타데이터(스키마 설명)만 저장합니다. 고객 데이터는 전송되지 않습니다. <!--The Audience export flow is done directly from Adobe Experience Platform Audience Portal (via [Destination](../connections/destinations.md)) to the customer database. The creation and update flow is done directly from your data warehouse database to Adobe Experience Platform Audience Portal.-->
...
페더레이션된 대상자 컴포지션은 데이터의 물리적 사본을 유지하지 않습니다. 이 데이터를 새로 고칠 빈도를 정의하기 위해 컴포지션에서 빈도를 구성합니다. 결과 대상자 데이터는 고객의 사용 사례 또는 액션에 필요한 기간 이상으로 Adobe Experience Platform에 저장되지 않습니다.

예:

* 대상자 생성의 경우 대상자가 데이터 웨어하우스에 생성되며, Adobe Experience Platform 대상자 포털을 통해 결과 대상자 및 관련 속성을 게시하기 전에 추가 컴포지션 작업 및 데이터 조작에 페더레이션된 대상자 컴포지션을 사용할 수 있습니다. 대상자 정의 및 관련 속성은 Adobe Experience Platform으로 전달됩니다.
...
결과 대상자 데이터는 Adobe Experience Platform 또는 페더레이션된 대상자 컴포지션에서 무기한 유지되지 않습니다. 사용 사례에 필요한 것보다 더 오래 보관되지 않습니다. 대상자 페이로드의 일부로 가져온 대상자 속성은 대상자 정의의 일부로만 유지됩니다. 지속 기간은 모든 대상자에 대한 TTL을 기반으로 하며 기본값은 30일입니다.
...
아니요, 컴포지션 중에는 ID 서비스가 활용되지 않습니다. 컴포지션에 사용된 다양한 소스 간의 데이터는 CRM ID, 사용자 계정 번호 등과 같은 사용자 정의 로직(기본 모델에 표시)을 통해 참여됩니다. 데이터 웨어하우스에서 선택할 대상자의 식별자로 사용되는 ID를 선택해야 합니다. 페더레이션된 대상자 컴포지션의 결과 대상자에서 결과 데이터 세트의 ID에 대한 ID 네임스페이스를 식별해야 합니다.

+++

## Federated Audience Composition을 사용하여 개인 정보 요청을 만들고 관리하려면 어떻게 해야 합니까?

+++ 답변

Adobe 페더레이션된 대상자 컴포지션에서 고객 데이터에 액세스하고 삭제하기 위해 두 가지 방법으로 개별 요청을 제출할 수 있습니다.
