...
| Update mode | 설명 |
| ----------- | ----------- |
| 전체 업데이트 | The full set of attributes in the selected schemas is updated for enrichment. |
| 증분 업데이트 | Only the fields that have been modified since the last enrichment ran are updated for the enrichment. |
...
>[!IMPORTANT]
>
>When the **Split** activity is executed, the population is separated across the different subsets in the **order they are added**. 예를 들어 첫 번째 하위 집합이 초기 모집단의 70%를 분할하면 다음 하위 집합은 나머지 30%에 선택 기준을 적용합니다.
>
>컴포지션을 실행하기 전에 분할을 실행할 순서대로 하위 집합의 순서를 지정했는지 확인하십시오.

After adding the **Split** activity to your composition, you can now determine how to subset your audience. **세그먼트 추가**&#x200B;를 선택하여 다른 분기 경로를 만듭니다.

You can now provide details for each of these sub-paths. 하위 경로에 필터 조건과 이름을 지정할 수 있습니다. To create a filtering condition, select **Create filter** and configure the filtering rule using the Query Modeler. Modeler 쿼리 사용에 대한 자세한 내용은 [Modeler 쿼리 개요](../query/home.md)를 참조하십시오.

필터링 조건을 만들면 다음 추가 규칙을 적용할 수 있습니다.

- **제한 사용**: 하위 집합으로 분할할 수 있는 프로필 수를 제한합니다. 모집단의 숫자 또는 백분율로 설정할 수 있습니다.
   - 제한을 활성화하면 특정 프로필 속성에 따라 선택한 프로필의 등급을 지정할 수도 있습니다. Turn on **Enable sorting**, and you can sort the attributes in ascending or descending order.
...
| 요일 | 컴포지션은 평일마다 실행됩니다. |
| 선택한 기간 | 컴포지션은 선택한 기간 동안 매일 실행됩니다. 반복 기간의 길이와 기간이 시작되는 날짜를 설정할 수 있습니다. |
| 선택한 요일 | The composition is executed every day of the week that is selected. |

Once you set the **Day of the month**, you can choose the start time. 모든 시간은 UTC로 표시됩니다.
...
지속 기간을 선택하면 대기 기간을 설정할 수 있습니다. This period of time can be in seconds, minutes, hours, or days.
...
컴포지션에서 전환은 데이터가 한 활동에서 다른 활동으로 전송되는 방법을 보여줍니다. The transitions store the data in a temporary work table. 전환을 선택하면 다음 정보를 볼 수 있습니다.
